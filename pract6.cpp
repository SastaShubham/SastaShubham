...
int flight::create()
{
	...
	do
	{
		...
		for(j=0;j<10;j++)
		{
			if(strcmp(city_index[j],s)==0)      //if source is already available then break
			break;                              //if both strings are equal then break the condition
		}
		if(j==10)
		{
			strcpy(city_index[city_count],s);   // If not presesnt then copy that source city at current index initially it is 0
			city_count++;
		}
